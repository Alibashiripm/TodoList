...
namespace CoreLayer.Services
{
	public abstract class BaseService
	{
		...
		protected TodoContext _context { get; private set; }

		protected BaseService(TodoContext context)
		{
			_context = context;
		}
		...
		#region Methods
		protected virtual async Task  Insert<T>(T entity) where T : BaseModel
		{
			if (entity == null)
			{
			...
			await Entities<T>().AddAsync(entity);
		}

		protected virtual async Task  Insert<T>(IEnumerable<T> entities) where T : BaseModel
		{
		...
		protected virtual async Task  Update<T>(T entity)
			where T : BaseModel
		{
			if (entity == null)
			{
		...
		protected virtual async Task  Update<T>(IEnumerable<T> entities)
			where T : BaseModel
		{
			if (entities?.Any() != true)
			{
		...
		protected virtual async Task  Delete<T>(T entity)
			where T : BaseModel
		{
			if (entity == null)
			{
		...
		protected virtual async Task  Delete<T>(IEnumerable<T> entities)
			where T : BaseModel
		{
			if (entities?.Any() != true)
			{
		...
		protected virtual IQueryable<T> Table<T>() where T : BaseModel
		{
		...
		protected virtual Task<T> GetById<T>(int entityId) where T : BaseModel
		{
			return Table<T>().SingleOrDefaultAsync(x => x.Id == entityId);
		}

		protected virtual Task<T> GetById<T>(long entityId, params string[] includs) where T : BaseModel
		{
